...
//                        buttons += '<a href="' + pathname + 'invoice/pdf/' + row.id + '/" target="_blank" class="btn btn-info btn-xs btn-flat"><i class="fas fa-file-pdf"></i></a> ';
    ...
    formatRowHtml: function (d) {
        ...
        html += '</thead>';
        html += '<tbody>';
        $.each(d.intoproduct, function (key, value) {
            html += '<tr>'
...
$(function () {
    ...
    $('#data tbody')
        ...
        .on('click', 'a[rel="details"]', function () {
            var tr = tblInto.cell($(this).closest('td, li')).index();
            var data = tblInto.row(tr.row).data();
        ...
        .on('click', 'a[rel="number"]', function () {
            ...
            var row = tblInto.row(tr);
            if (row.child.isShown()) {
                row.child.hide();
                tr.removeClass('shown');
            } else {
                row.child(Into.formatRowHtml(row.data())).show();
                tr.addClass('shown');
            }
        });
