...
//            '¿Estas seguro de crear al siguiente solicitante?', parameters, function (response) {
        ...
        ajax: {
            ...
            data: function (params) {
                return {
                    ...
                    ids: JSON.stringify(into.getProductsIds())
                };
            },
            processResults: function (data) {
                return {
    ...
    $('#tblProducts tbody')
        ...
        .on('click', 'a[rel="remove"]', function () {
            ...
            alert_action('Notificación', '¿Estas seguro de eliminar el insumo de tu detalle?',
                function () {
                    into.details.products.splice(tr.row, 1);
                    tblProducts.row(tr.row).remove().draw();
                    into.calculateInvoice();
                }, function () {
        ...
        .on('change', 'input[name="cant"]', function () {
            ...
            var tr = tblProducts.cell($(this).closest('td, li')).index();
            into.details.products[tr.row].cant = cant;
            into.calculateInvoice();
            $('td:last', tblProducts.row(tr.row).node()).html('Q' + into.details.products[tr.row].subtotal.toFixed(2));
        });

    $('.btnRemoveAll').on('click', function () {
        if (into.details.products.length === 0) return false;
        alert_action('Notificación', '¿Estas seguro de eliminar todos los details de tu detalle?', function () {
            into.details.products = [];
            into.listProducts();
        }, function () {
...
//    }).val(0.12);
    ...
    $('#frmInto').on('submit', function (e) {
        e.preventDefault();

        if (into.details.products.length === 0) {
...
//        });
        ...
        var parameters = new FormData(this);
        parameters.append('products', JSON.stringify(into.details.products));
        submit_with_ajax(pathname, 'Notificación',
